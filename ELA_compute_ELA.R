...
# un-comment this line, if running in unix AND require personal library
...
crash_label <- json_data$crash_label
...
func_base <- function(prob_type, list_sheet, list_input, list_obj, list_ELA){
  ...
  if (all(prob_type=="AF")){ # artificial functions
    for (sheet_i in 1:length(list_sheet)){
      ...
      filepath <- paste(filepath_save, path_split, crash_label, '_', prob_type, '_', sheetname_temp, '.xlsx', sep="")
      ...
      filename_temp <- paste(filepath_save, path_split, "featELA_", crash_label, "_", prob_type, '_', sheetname_temp, ".xlsx", sep='')
      saveWorkbook(wb, file=filename_temp, overwrite=TRUE)
      print(paste("Sheet ", sheetname_temp, " done!", sep=""))
    }
    
  } else { # crash and BBOB
    filepath <- paste(filepath_save, path_split, crash_label, '_', prob_type, '.xlsx', sep="")
    wb <- createWorkbook()
    ...
    filename_temp <- paste(filepath_save, path_split, "featELA_", crash_label, "_", prob_type, ".xlsx", sep='')
    saveWorkbook(wb, file=filename_temp, overwrite=TRUE)
  } # END IF
  print(paste("ELA ", prob_type, " ", crash_label, " done.", sep=""))
} # END DEF
