...
import numpy as np
import pandas as pd
from .RGF import rgf_generate_tree as genTree
from .RGF import rgf_tree2func as genTree2func
...
class func_generator:
    def __init__(self,
        ...
        self.path_output = os.path.join(path_base, 'results_rgf')
    ...
    def __call__(self):
        ...
        while (len(self.df_func) < self.f_number):
            i_run += 1
            ...
            df_f = pd.DataFrame.from_dict({'str_f': [str_f], 'y': [str(list(y))]})
            self.df_func = pd.concat([self.df_func, df_f], axis=0, ignore_index=True)
        filepath = os.path.join(self.path_output, 'rgf_genf.csv')
        self.df_func.to_csv(filepath, index=False)
        if (self.verbose):
            print(f'[RGF] Done. Valid: {len(self.df_func)}; Invalid: {i_invalid}; Bad y-values: {i_bad_y}; Total runs: {i_run}; Success rate: {round(len(self.df_func)/i_run, 2)}')
# END DEF