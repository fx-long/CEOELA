...
def generate_tree(minlen, maxlen):
    ...
    pOperator = [15, 15, 10, 10, 2, 2, 2, 5, 5, 
                 2, 2, 3, 3, 3, 3, 3, 3, 1, 1, 1]
    ...
    # Randomly generate a tree
    for i in range(np.random.randint(minlen, maxlen)):
        # Randomly reach a leaf
        p = tree
        while (p.get_type() > 0):
            if ((p.get_type() == 1) or (np.random.random() < 0.5)):
                p = p.left
            else:
                p = p.right
            # END IF
        ...
        # Select an operator
        operator = int(mOperator[np.argwhere(np.random.random() <= pOperator)[0][0]])
        # Binary operator
        if (operator <= 20):
            # Select an operand
            operand = int(mOperand[np.argwhere(np.random.random() <= pOperand)[0][0]])
