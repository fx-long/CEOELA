...
#%%
def tree2func(tree, dim_x, dim_y):
    exp = generate_tree2exp(tree)
    str_f = generate_exp2fun(exp, dim_x, dim_y)
    ...
    for item in exp_flat:
        if (item < 0):
            ...
            if (item == 1):
                ...
                str_main.append('(np.hstack((x[1:].ravel(), np.zeros((1, 1)).ravel())))')
                ...
                # Index vector
                str_main.append(f'(np.array(range(1, {dim_y}+1)))')
                
            elif (item == 7):
                # Random number in 1-1.1
                str_main.append(f'(1+np.array({np.random.random()})/10)')
                ...
                # Sum of vector
                str_main = str_main[:-1] + ['np.sum(' + str_main[-1] + ')']
                
            elif (item == 33):
                # Mean of vector
                str_main = str_main[:-1] + ['np.mean(' + str_main[-1] + ')']
                
            elif (item == 34):
                # Cumulative sum of vector
                str_main = str_main[:-1] + ['np.cumsum(' + str_main[-1] + ')']
                
            elif (item == 35):
                # Product of vector
                str_main = str_main[:-1] + ['np.prod(' + str_main[-1] + ')']
                
            elif (item == 36):
                # Maximum of vector
                str_main = str_main[:-1] + ['np.amax(' + str_main[-1] + ')']
